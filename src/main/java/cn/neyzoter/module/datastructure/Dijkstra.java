...
public class Dijkstra {
     ...
     */
    public int[] getShortestPaths(int[][] adjMatrix, int s) {
        int[] result = new int[adjMatrix.length];   //用于存放顶点s到其它顶点的最短距离
        boolean[] used = new boolean[adjMatrix.length];  //用于判断顶点是否被遍历
        used[s] = true;  //表示顶点s已被遍历
        for(int i = 1;i < adjMatrix.length;i++) {
            result[i] = adjMatrix[s][i];
            used[i] = false;
        }
    ...
    public static void main(String[] args) {
        ...
        int[][] adjMatrix = {{0,6,3,-1,-1,-1},
                ...
                {-1,-1,4,2,0,5},
                {-1,-1,-1,3,5,0}};
        int[] result = test.getShortestPaths(adjMatrix, 0);
